...
Code review of story-2.2 (2026-07-24) — 3 layers (Blind Hunter, Edge Case Hunter, Acceptance Auditor), all completed. Acceptance Auditor: all 4 ACs met with genuine offline tests. Adversarial layers found real hardening gaps (two empirically reproduced). **8 patch, 4 defer, 0 decision-needed. Verdict: changes requested.**

- [x] [Review][Patch] **Form-filter parity with the mart — work-list silently misses 10-KT/legacy periodic reports.** `get_filings(form=["10-K","10-Q"])` does EXACT matching (fetches only `{10-K,10-Q,10-K/A,10-Q/A}`), but the mart uses `startsWith(form,'10-K') OR startsWith(form,'10-Q')` (also `10-KT`, `10-KSB`, `10-K405`, `10-QT`, …). A company filing only a `10-KT` in the window (routine on a fiscal-year-end change) has authoritative facts in the mart but is never proposed → permanently missing. Fix: fetch without a `form=` filter and filter locally by the SAME prefix family (`startswith 10-K / 10-Q`) as the mart; correct the false "matches the mart" comment. [fintin/adapters/edgar/filings_index.py]
- [x] [Review][Patch] **Membership must be by accession, not date-windowed (AD-16 fidelity).** `present_accessions` filters `filed_date >= since`, coupling the membership authority to a date; an already-present accession whose stored `filed_date` is just below `window_start` (while its index `filing_date` is in-window) flips to a false "outstanding." Fix: check membership by the exact candidate accessions (`WHERE accession IN (candidates)`) — date-independent, and efficient (accession is `raw_fact`'s leading sort key). The window sizes only the index fetch (a scan hint, per AD-16). Reorder the CLI to fetch candidates → query membership by their accessions → diff. [fintin/adapters/store/raw_fact_repo.py present_accessions, fintin/cli/app.py]
- [x] [Review][Patch] **A config-valid CIK above int32 max crashes the pyarrow filter.** `pa.array(sorted(ciks), type=cik_type=int32)` raises `ArrowInvalid` for a CIK > 2,147,483,647 (config accepts up to the UInt32 store max 4,294,967,295), taking down the whole command with an opaque error. Latent today (max real CIK ≈ 2.1M) but a store/index width mismatch. Fix: cast the index `cik` column to int64 and build the value-set as int64. [fintin/adapters/edgar/filings_index.py]
- [x] [Review][Patch] **`lookback_days` has no upper bound → OverflowError / quarterly-fetch fan-out.** `_parse_reconcile` only rejects `< 1`; a huge value makes `today - timedelta(days=…)` raise `OverflowError` (reproduced ≈739,821) and a merely-large value fans one-request-per-quarter into dozens of index downloads. Fix: cap at a sane max (e.g. 3650 days ≈ 10y) in `_parse_reconcile`. [fintin/config.py]
- [x] [Review][Patch] **Drop null/malformed index rows; make the "validates" docstring honest.** A null `filing_date` → `WorkItem.filed_date = None` → `compute_work_list` sort `TypeError` and `--show-items` `isoformat` `AttributeError`; `normalize_accession` is pass-through (no `_ACCESSION_RE` check), so a malformed accession becomes a permanent phantom work item. Fix: in `_filings_to_work_items`, drop rows with a null `filing_date`/`accession` or an accession failing `_ACCESSION_RE`. [fintin/adapters/edgar/filings_index.py]
- [x] [Review][Patch] **Deterministic co-filed dedup + honest scope docstring.** `compute_work_list` keeps whichever co-filer CIK the index listed first (arbitrary/order-dependent — kboss values determinism); dedup by keeping `min(cik)`. Separately, correct the adapter docstring's false claim that excluded forms "never produce Tier 0 facts" (20-F/40-F carry XBRL) — note foreign annual forms are out of v1 scope. [fintin/core/reconcile.py, fintin/adapters/edgar/filings_index.py]
- [x] [Review][Patch] **Clamp the scan window against a future-dated HWM.** A corrupt `raw_fact` row with `filed_date > today` → `hwm > today` → `window_start > window_end` (a reversed `"A:B"` range of unverified behavior). Fix: `anchor = min(hwm, today)` in `resolve_window`. [fintin/core/reconcile.py]
- [x] [Review][Patch] **Company count / display robustness under the reorder.** After the AD-16 reorder, keep the `N outstanding across M companies` summary accurate (distinct CIKs among the deduped work items) and `--show-items` safe for 0 items. [fintin/cli/app.py]
- [x] [Review][Defer] **Foreign-issuer support (20-F/40-F).** These carry XBRL financial statements but are foreign private issuers — out of v1 scope (ARCHITECTURE-SPINE.md Deferred: "Foreign/IFRS filers — Won't (v1)"). Revisit the form family + mart filter if the Universe expands to foreign filers. — deferred, out of v1 scope.
- [x] [Review][Defer] **`date.today()` is local, not US-Eastern.** Near midnight/quarter boundary the window can name a day EDGAR hasn't reached; self-healing via the lookback + per-run re-derivation. Not worth a timezone dependency in v1. — deferred, self-healing.
- [x] [Review][Defer] **A 429 mid-multi-quarter re-fetches earlier quarters.** `EdgarClient.run` retries the whole `get_filings` op, re-downloading already-fetched quarters during a cool-down. Rate-safe, just redundant. — deferred, low value.
- [x] [Review][Defer] **FR-2 "one fetch per quarter" not unit-asserted.** Delegated to edgartools internals; confirmed only by the live smoke (NFR-7 bans live tests). — deferred, inherent coverage limit.
...
- 2026-07-24 — Code review (3 layers, all completed). Acceptance Auditor: all 4 ACs met. Applied all 8 patch findings: (1) **form-family parity with the mart** — fetch all forms and prefix-filter `10-K*`/`10-Q*` locally (`get_filings`' exact `form=` matching was dropping `10-KT`/legacy periodics the mart counts); (2) **membership by exact accession** — `present_accessions(accessions=…)` decoupled from any date (AD-16 authority; also efficient — accession is `raw_fact`'s leading sort key); CLI reordered fetch→membership→diff; (3) **int64 CIK filter** — a config-valid >int32 CIK no longer overflows the pyarrow value-set; (4) **`lookback_days` cap** (`MAX_LOOKBACK_DAYS=3650`) — guards the timedelta OverflowError + quarterly fan-out; (5) drop null/malformed index rows + validate accession against `_ACCESSION_RE` (honest "validates" docstring); (6) **deterministic co-filed dedup** (keep min CIK) + corrected the false "excluded forms carry no facts" docstring (20-F/40-F do — but are out of v1 scope); (7) clamp a future-dated HWM in `resolve_window`; (8) company-count/display robustness. 4 defers (foreign 20-F/40-F support; local-vs-ET `today`; 429 mid-range re-fetch; FR-2 quarter-count assertion) logged in deferred-work.md. **213 tests pass** (+6); re-verified live on Apple (full-index fetch → prefix+CIK filter → accession membership → 0 outstanding, exit 0). Status → done. Files also touched: `fintin/core/reconcile.py`, `fintin/adapters/edgar/filings_index.py`, `fintin/adapters/store/raw_fact_repo.py`, `fintin/cli/app.py`, `fintin/config.py`, `tests/test_reconcile.py`, `tests/test_filings_index.py`, `tests/test_raw_fact_repo.py`, `tests/test_config.py`.
