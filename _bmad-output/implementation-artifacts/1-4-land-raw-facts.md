...
_Adversarial code review (2026-07-24) — 3 parallel layers (Blind Hunter, Edge Case Hunter, Acceptance Auditor), all verified against the installed edgartools 5.43.0 source; suite offline. Triage: 0 decision-needed, 9 patch, 1 deferred, 2 dismissed. All 5 ACs functionally satisfied; no defect in the filter/map logic — findings are robustness + test-coverage gaps. Confirmed-correct: `numeric_value` is the actual unscaled `val`; `raw_tag` keeps the namespace; `core` imports no `edgar` (guard passes); `RawFactRow` order == `RAW_FACT_COLUMNS` == DDL._

- [x] [Review][Patch] (medium) `edgar.get_company_facts` returns **`None`** (not raises) on an empty/factless/unparseable response; `fetch_company_facts` passes it through → `to_raw_fact_rows(None)` → `TypeError: 'NoneType' object is not iterable`, surfaced as a cryptic "Ingest failed". Handle `None` in `fetch_company_facts` with a clear typed error (no companyfacts for CIK N). [fintin/adapters/edgar/facts.py]
- [x] [Review][Patch] (medium) Ingest `version` uses wall-clock `time.time_ns()`, which can step **backward** (NTP/manual/VM) → a corrected re-ingest could get a lower version and lose under `ReplacingMergeTree`, defeating AD-6 supersession. Derive the version from the store (`max(version)+1`): add `next_ingest_version(client)` to the repo, have the CLI supply it, and expose `version` on `IngestResult`. [fintin/core/ingest.py, fintin/adapters/store/raw_fact_repo.py, fintin/cli/app.py]
- [x] [Review][Patch] (medium) All rows of a run share one `version`, so an intra-batch duplicate identity key `(accession, raw_tag, period_start, period_end, unit)` with differing values dedups **nondeterministically** under `ReplacingMergeTree`. De-duplicate within the batch deterministically (last-wins) + count/log collisions. [fintin/core/ingest.py]
- [x] [Review][Patch] (medium) Accession validation: a whitespace-only `accession` passes the `not f.accession` guard and normalizes to `""` (empty identity anchor); non-canonical shapes pass unchanged. Reject anything not matching `\d{10}-\d{2}-\d{6}` → `dropped_incomplete`. [fintin/core/ingest.py]
- [x] [Review][Patch] (medium) Non-finite `numeric_value` (`NaN`/`inf`) passes the `is None` filter and lands in `Float64` (hashes as `'nan'`, invisible corruption; poisons downstream math). Drop with `math.isfinite`. [fintin/core/ingest.py]
- [x] [Review][Patch] (medium) No CIK domain check — `cik: int` accepts negative/zero/> UInt32-max, wasting an EDGAR fetch before a ClickHouse insert error/truncation. Validate `1 <= cik <= 4294967295` in the CLI **before** building the client/fetching. [fintin/cli/app.py]
- [x] [Review][Patch] (low) Period handling infers instant from `period_start is None` (not `period_type`); a duration with a missing start would be silently collapsed to a zero-length period, and reversed/zero-length durations land unflagged. Add `period_type` to `FactLike`; collapse only instants; drop durations with missing start or `period_start >= period_end`. [fintin/core/ingest.py]
- [x] [Review][Patch] (low) `content_hash` omits `raw_label` and relies on an unenforced `\x1f`-separator assumption. Include `raw_label` and use an injection-proof encoding (hash `repr((...fields...))`) so no field value can forge a collision. [fintin/core/ingest.py]
- [x] [Review][Patch] (medium) Test gaps the spec's Task 2/Task 6 require but that were checked-without-implementing: (a) a **fixture-based** test parsing a hand-crafted companyfacts JSON via `EntityFactsParser.parse_company_facts` → `to_raw_fact_rows`, pinning a known value (scale-regression guard for AC-1); (b) an **end-to-end** test joining `ingest_company`/`to_raw_fact_rows` output to a real throwaway-DB insert + read-back (the transform→insert seam); (c) a real `next_ingest_version` monotonicity assertion (the current "monotonic_ns" test asserts nothing about monotonicity). [tests/test_ingest.py, tests/test_raw_fact_repo.py]
- [x] [Review][Defer] (low) ClickHouse `Date` range (1970-01-01 … 2149-06-06) is unguarded — an out-of-range/corrupt date would clamp/error and desync `content_hash`. [fintin/adapters/store/schema.py, fintin/core/ingest.py] — deferred, unreachable for real SEC/XBRL dates (post-2009); revisit with a validation/scrub pass or `Date32`.

_Dismissed (2): `rows_landed` reflects rows built vs. inserted — insert is all-or-nothing (raises on failure), so counts match on success; `fetch_company_facts` relies on `EdgarClient` having configured edgar's global identity/rate — construction is the enforced prerequisite (you need a client to pass) and the structural guard confines `get_company_facts` to `facts.py`._
...
- 2026-07-24 — Code review (adversarial, 3 layers, verified against installed edgartools 5.43.0; offline): 9 patches applied, 1 deferred, 2 dismissed, 0 decision-needed. **Robustness fixes:** `fetch_company_facts` raises a clear `NoCompanyFactsError` on edgar's `None` return (was a cryptic `TypeError`); ingest `version` now comes from `next_ingest_version` = `max(version)+1` in the store, not a wall clock (AD-6 supersession no longer defeatable by a clock step-back); intra-batch identity-key collisions de-duplicated deterministically (last-wins); accession validated against `\d{10}-\d{2}-\d{6}` (whitespace no longer bypasses the guard); non-finite `numeric_value` (NaN/inf) dropped; CIK range-validated in the CLI before any fetch; `period_type`-aware instant/duration (reversed/zero-length/missing-start durations dropped); `content_hash` includes `raw_label` + injection-proof `repr`-tuple encoding. **Test gaps closed:** fixture-based parse→transform scale guard, end-to-end transform→insert→read-back seam, real `next_ingest_version` monotonicity. **Deferred:** ClickHouse `Date` range guard (unreachable for SEC dates). Suite: **96 passed**. Status → done.
