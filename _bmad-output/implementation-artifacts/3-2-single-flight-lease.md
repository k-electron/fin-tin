...
Reviewed 2026-07-24 by 3 parallel adversarial layers (Blind Hunter, Edge Case Hunter, Acceptance Auditor), all completed. **All 7 ACs + 9 settled decisions verified genuinely satisfied.** The concurrency-focused layers found real robustness gaps in the filesystem lease (no correctness defect on the *common* paths — free acquire, live-lease coalesce, and error-path release are race-free and tested — but the *reclaim* path could steal a live lease under load). Triage: **7 patch, 2 defer, 4 dismissed.**

- [x] [Review][Patch] **Reclaim on *unconfirmed* death could steal a live lease (a lock that releases itself under load)** — `_read_record` caught a blanket `OSError` → `None`, and `acquire` treated `None` as reclaimable. So a transient read failure on a **live** lease (fd exhaustion `EMFILE`, `EIO`, or a misconfigured directory-path) let a second run reclaim and start a concurrent EDGAR run (the double-rate ban this feature prevents). Fixed with `_existing_is_reclaimable`: reclaim **only** on positive evidence of death — vanished (`FileNotFoundError`), corrupt *content* (`JSONDecodeError`/non-dict; atomic writes ⇒ never a torn read), or a readable record positively past its TTL. A present-but-unreadable file is presumed **live** and coalesced. [fintin/adapters/lease/file_lease.py; test `test_unreadable_lease_is_presumed_live_not_stolen`]
- [x] [Review][Patch] **`os.link` non-`FileExistsError` unhandled + `.tmp` leak on every attempt** — on a filesystem without hard-link support (`EXDEV`/`EPERM`/network mounts) `os.link` raised a bare `OSError` that propagated as a misleading "Catch-up failed", and the just-written temp record leaked on every attempt. Now the temp write+link is wrapped so the `.tmp` is always removed in a `finally`, and a non-`FileExistsError` `os.link` error is re-raised with a clear "cannot create the single-flight lease at …" message. [fintin/adapters/lease/file_lease.py]
- [x] [Review][Patch] **`_unlink_quietly` only suppressed `FileNotFoundError`** — a different unlink error (read-only dir, path-is-a-directory, permission) could crash `acquire` or, worse, propagate from `release()`'s `finally` and **mask the run's real exception** (a throttle would surface as a confusing unlink error). Broadened to log-and-swallow all `OSError`. [fintin/adapters/lease/file_lease.py]
- [x] [Review][Patch] **Heartbeat could write after release (join-timeout window)** — added a `self._stop.is_set()` check at the top of `_touch`, so the daemon thread does not rewrite the lease once `release()` has begun (narrows the "heartbeat recreates the file after unlink" phantom-lease window). [fintin/adapters/lease/file_lease.py]
- [x] [Review][Patch] **No TTL ceiling → a fat-fingered `ttl_seconds` defeats self-expiry** — `ttl_seconds` was only floored at `>= 2`; a typo like `8640000` would honor a crashed run's lease for ~100 days, silently breaking "a crashed run never deadlocks the tool". Added `MAX_LEASE_TTL_SECONDS = 86400` (24h) and validate `2 <= ttl <= MAX`. [fintin/config.py; new config-reject test]
- [x] [Review][Patch] **Flaky heartbeat-freshness test** — the 0.3s TTL / 0.05s heartbeat / 0.5s sleep left only a 0.2s margin, so a scheduling hiccup on a loaded CI could spuriously cross the TTL and reclaim. Widened to 1.0s TTL / 0.1s heartbeat / 1.3s sleep (10 beats per TTL, a full second of slack). [tests/test_lease.py]
- [x] [Review][Patch] **README understated the lease record contents** — said `{pid, host, timestamps}`; the record also carries `token` and `ttl`. Corrected (still no PII). [README.md]
- [x] [Review][Defer] **The in-process heartbeat protects the lease *file*, not the *run*** [fintin/adapters/lease/file_lease.py] — a holder process frozen past the TTL (laptop suspend, `SIGSTOP`, a forward wall-clock jump since staleness uses `time.time()`, not `monotonic`) can be displaced mid-run, and the resumed run doesn't re-verify ownership → two concurrent runs. Inherent to a single-machine filesystem lease with an in-process heartbeat; accepted for v1 (120s TTL bounds it; displaced writes are still idempotent-by-version, AD-6 — wasted requests, never corruption). Deferred-work expanded to cover this and the heartbeat-vs-reclaim `os.replace` "flapping" variant; a robust fix needs fencing tokens / `flock`.
- [x] [Review][Defer] **`.tmp` record files leak on a hard crash** [fintin/adapters/lease/file_lease.py] — one uniquely-named temp per `SIGKILL`/power-loss between write and the `finally` unlink; bounded but never GC'd. Deferred (sweep stale `<lease>.*.tmp` on acquire if litter accrues).

_Dismissed (4, with rationale): (a) **`FileLease.__init__` doesn't enforce `2*hb<=ttl`** — the config path validates it, and the tests **deliberately** construct `hb > ttl` (`ttl=0.1, heartbeat=30`) to simulate a crashed non-beating holder, so an `__init__` assert would break that legitimate tool; the CLI never bypasses config. (b) **`0o600` lease-file mode not enforced** — the record holds no PII/secrets, so the mode is cosmetic. (c) **FileLease tests don't `release()` in `finally`** — the heartbeat threads are daemons that die at process exit (no hang); wrapping every test is churn for negligible benefit. (d) **`acquire` retry-exhaustion labels the result `ALREADY_RUNNING`** — mooted by the fail-safe reclaim: exhausting the 3 retries now means another run kept winning the reclaim, so coalescing is correct._
...
- 2026-07-24 — Code review (3 parallel adversarial layers; concurrency-focused). All 7 ACs + 9 decisions verified satisfied; the common paths (free acquire, live-lease coalesce, error-path release) are race-free. Applied all **7 patch findings**, the key one being a fail-safe reclaim fix — `FileLease` now reclaims **only** on positively-confirmed death (stale/corrupt-content/vanished), presuming a present-but-unreadable lease **live** so a transient read error (fd exhaustion, a directory path) can't steal a live lease and cause a concurrent double-EDGAR run; plus robust temp cleanup + clear hard-link-unsupported error, `_unlink_quietly` broadened (can't mask the run's exception), a `_touch` post-release guard, a `MAX_LEASE_TTL_SECONDS` ceiling, a widened flaky timing test, and a README fix. **2 deferred** (the inherent in-process-heartbeat-vs-process-freeze/clock-jump reclaim limitation — accepted for v1, degrades to idempotent wasted requests; `.tmp` litter on SIGKILL) and **4 dismissed** (`__init__` hb<=ttl guard would break the deliberate crashed-holder test; 0o600 no-secrets; test daemon-thread hygiene; retry-exhaustion label mooted by the fail-safe fix), logged. **311 tests pass (+2)**. Secret-scan clean. Status → done.
