...
- [x] **Task 1 — Pure catch-up engine** (AC: 1, 2, 3, 4) — `fintin/core/catchup.py` (NEW, pure)
  ...
  - [x] Import only: `enum`, `NamedTuple`, `Callable`/`Sequence`; `from fintin.core.reconcile import WorkList`; `from fintin.core.backfill import BackfillReport, BackfillStrategy, backfill_universe`; `from fintin.core.ingest import RawFactRow` (for the insert type). **No `edgar`/ClickHouse/`pyarrow`.**
