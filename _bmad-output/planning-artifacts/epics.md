...
**Given** a versioned **concept dictionary** (each screening concept = an ordered list of standard elements: FASB-primary + observed-frequency-ranked fallbacks) **When** the mart resolves a concept for a `(cik, period)` **Then** it returns the value of the **first present** element in that list — deterministic across synonymous elements, so multiple elements collapsing to one screening concept never produce a nondeterministic value (AD-8, AD-9).
